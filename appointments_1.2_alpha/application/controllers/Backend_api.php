...
use \EA\Engine\Types\SafeString; 
    ...
    public function ajax_save_appointment() {
        try {
            ...
            try {
                ...
                if (!$manage_mode) {
                    $customer_title = new SafeString($this->lang->line('appointment_booked'));
                    $customer_message = new SafeString($this->lang->line('thank_you_for_appointment'));
                    $provider_title = new SafeString($this->lang->line('appointment_added_to_your_plan'));
                    $provider_message = new SafeString($this->lang->line('appointment_link_description'));
                } else {
                    $customer_title = new SafeString($this->lang->line('appointment_changes_saved'));
                    $customer_message = new SafeString('');
                    $provider_title = new SafeString($this->lang->line('appointment_details_changed'));
                    $provider_message = new SafeString('');
                }
            ...
            try {
                ...
                if ((bool)$send_provider === TRUE) {
                    $email->sendDeleteAppointment($appointment, $provider,
                            ...
                            new SafeString($_POST['delete_reason']));
                }
